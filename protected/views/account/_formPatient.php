...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="form-group row">
		<div class="col-sm-12" style="text-align:right">
			<?php if(Yii::app()->user->name == 'super admin' || Yii::app()->user->name == 'admin') {
			 	echo CHtml::link('<span class="icon text-white-50"><i class="fas fa-user-times"></i></span><span class="text">Cancel</span>', $this->createAbsoluteUrl('account/listPatient'), array('class'=>'btn btn-danger btn-icon-split', 'onclick'=>'return confirm("Are you sure you want to cancel creating an account?")')); 
				echo CHtml::link('<span class="icon text-white-50"><i class="fas fa-user-check"></i></span><span class="text">Save</span>', 'javascript:document.getElementById("account-form").submit();', array('class'=>'btn btn-success btn-icon-split')); 
			?>
			<?php } else {
				echo CHtml::link('<span class="icon text-white-50"><i class="fas fa-user-times"></i></span><span class="text">Cancel</span>', $this->createAbsoluteUrl('account/listPatientSec'), array('class'=>'btn btn-danger btn-icon-split', 'onclick'=>'return confirm("Are you sure you want to cancel creating an account?")')); 
				echo CHtml::link('<span class="icon text-white-50"><i class="fas fa-user-check"></i></span><span class="text">Save</span>', 'javascript:document.getElementById("account-form").submit();', array('class'=>'btn btn-success btn-icon-split'));
			} ?>
		</div>
	</div>
...
<script>
    ...
    $('#User_dob').on('change', function() {
        ...
        $.ajax({
            url: "./CheckAge", // Adjust if your Yii app is in a subdirectory
            type: 'POST',
            ...
            success: function(response) {
                if (response.age < 18) {
				   ...
				   //$('#birthHistoryForm input, #birthHistoryForm select, #birthHistoryForm textfield, #birthHistoryForm button').prop('disabled', false);
                } else {
                    $('#birthHistoryForm').hide();
				   //$('#birthHistoryForm input, #birthHistoryForm select, #birthHistoryForm textfield, #birthHistoryForm button').prop('disabled', true);
                }
            },
            error: function() {
