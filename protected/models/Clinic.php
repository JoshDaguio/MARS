...
class Clinic extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('clinic, address, contact_number, status_id', 'required'),
			array('status_id', 'numerical', 'integerOnly'=>true),
			array('clinic, address, contact_number', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, clinic, address, contact_number, status_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'clinicAssignments' => array(self::HAS_MANY, 'ClinicAssignment', 'clinic_id'),
		);
	}
